{lib, pkgs, ...}: let
  ...
  phone-connect = pkgs.writeShellScriptBin "phone-connect" ''
    set -uo pipefail
    ...
    check_avahi() {
      ...
      while IFS=';' read -r event iface proto name stype domain hostname ip port txt; do
        [[ -z "$event" ]] && continue
        [[ "$event" != "=" ]] && continue
        [[ "$proto" != "IPv4" ]] && continue
  ...
  phone-connect-service = pkgs.writeShellScriptBin "phone-connect-service" ''
    set -uo pipefail
    ...
    main() {
      ...
      local new_addr
      if new_addr=$(${phone-connect}/bin/phone-connect 2>/dev/null); then
...
in {
  ...
  # Replace the always-running daemon with a timer-based service
